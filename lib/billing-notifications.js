...
// The day a cancelled family's data is wiped, as the win-back email states it.
// Must match GRACE_PERIOD_MS in api/purge-canceled-accounts.js (60 days from
// canceled_at); the cron runs at 03:00 UTC, so "on" that day is accurate.
const PURGE_GRACE_DAYS = 60;
function purgeDateFrom(canceledAtIso) {
  const start = canceledAtIso ? new Date(canceledAtIso) : new Date();
  return formatDate(new Date(start.getTime() + PURGE_GRACE_DAYS * 86400000).toISOString());
}
...
async function sendLoopsEvent(email, eventName, properties) {
  ...
  try {
    const response = await fetch('https://app.loops.so/api/v1/events/send', {
      ...
      // Loops' events/send endpoint doesn't have a `contactProperties` field —
      // contact properties (like firstName, used by {contact.firstName} in
      // workflow emails) must be plain top-level keys on the request body, or
      // Loops stores them as one garbage custom property literally named
      // "contactProperties" holding a JSON string, instead of updating the
      // real fields. Spreading here alongside eventProperties keeps both
      // {eventProperties.x} and {contact.x} template variables working.
      body: JSON.stringify({ email, eventName, eventProperties: properties || {}, ...(properties || {}) }),
    });
    if (!response.ok) {
...
//
// fbp, fbc, clientIp and clientUserAgent are the browser-side match keys (see
// lib/meta-attribution.js). Meta wants them as-is, not hashed, and they're only
// ever passed for a visitor who accepted cookies. testEventCode routes the event
// to Events Manager > Test Events instead of counting it.
//
// Returns { sent, status } so the caller can log the outcome.
async function sendMetaCAPIEvent({ eventName, eventId, email, userId, value, currency, eventSourceUrl, fbp, fbc, clientIp, clientUserAgent, testEventCode }) {
  if (!process.env.META_CAPI_ACCESS_TOKEN) return { sent: false, status: 'no_token' };
  const userData = {};
  ...
  const payload = { data: [event], access_token: process.env.META_CAPI_ACCESS_TOKEN };
