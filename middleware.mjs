...
export default function middleware(request) {
  ...
  // /join/GREENFIELD25 is a partner's short link — the version that gets printed
  // on a poster, read out at assembly, or hidden behind a QR code. The code is
  // banked into a cookie here and the visitor is sent on to a clean homepage
  // URL, so nothing ugly is left in the address bar and the homepage's relative
  // image/nav paths still resolve from the site root.
  //
  // Setting it server-side rather than from a script also makes it outlast
  // Safari's seven-day cap on script-written storage, which is the single most
  // likely way a school signup would otherwise lose its attribution.
